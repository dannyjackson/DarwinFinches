...
def main():
    ...
    ap.add_argument("--offset", type=int, default=None, help="Generations between t1 and t2 (for reporting).")
    ...
    if args.tsv_out is not None:
        header = "\t".join([
            "rep", "sel_s", "decline_rate", "offset", "model", "seed",
            "num_sites", "num_mutations",
            "pi_t1", "pi_t2", "dxy", "fst_hudson",
            "tajd_t1", "tajd_t2", "delta_tajd"
        ])
        row = "\t".join(map(str, [
            args.rep, args.sel_s, args.decline_rate, args.offset, args.model, args.seed,
