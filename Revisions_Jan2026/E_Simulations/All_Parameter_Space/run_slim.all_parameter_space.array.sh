...
# Parse arguments
# ----------------------------
while getopts "s:r:f:m:N:o:L:" option; do
    case "${option}" in
        s) S=${OPTARG} ;;
...
echo "JOB=$SLURM_JOB_ID  ARRAY_TASK=$SLURM_ARRAY_TASK_ID  REP=$REP  S=$S  DECLINE_RATE=$DECLINE_RATE  F0=$F0  NE=$NE  OFFSET=$OFFSET  L=$SEG_LEN"
echo "PWD=$PWD"
